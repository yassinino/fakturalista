...
<head>
    ...
    <title>{{ $daysLeft === 1 ? __('emails.trial_reminder.subject_1_day') : __('emails.trial_reminder.subject_3_days') }}</title>
    <!--[if mso]>
    <noscript>
...
<div class="email-card">
    ...
    <div class="hero">
        ...
        <div class="hero-title">
            {{ $daysLeft === 1 ? __('emails.trial_reminder.hero_title_1_day') : __('emails.trial_reminder.hero_title_3_days') }}
        </div>
        <div class="hero-subtitle">{{ __('emails.trial_reminder.hero_subtitle') }}</div>
    </div>
    ...
    <div class="body">
        ...
        <p class="greeting">
            {{ __('emails.trial_reminder.greeting_hello') }} <strong>{{ $tenant->owner_name }}</strong>,<br><br>
            @if ($daysLeft === 1)
                {{ __('emails.trial_reminder.body_1_day', ['company' => $tenant->company_name]) }}
            @else
                {{ __('emails.trial_reminder.body_3_days', ['company' => $tenant->company_name]) }}
            @endif
        </p>
        ...
        <div class="cta-wrapper">
            <a href="{{ $subscribeUrl }}"
               ...
               style="display:inline-block;background:#fa7070;color:#ffffff;text-decoration:none;font-size:15px;font-weight:700;letter-spacing:0.02em;padding:14px 36px;border-radius:10px;">
                {{ __('emails.trial_reminder.cta_button') }}
            </a>
        </div>
        ...
        {{-- Help --}}
        <p class="help-text">
            {{ __('emails.trial_reminder.help_question') }}
        </p>
        <p class="help-text">
            📧 {{ __('emails.trial_reminder.help_contact') }} <a href="mailto:contact@fakturalista.com">contact@fakturalista.com</a>
            {{ __('emails.trial_reminder.help_response') }}
        </p>
    ...
    {{-- ── Footer ───────────────────────────────────── --}}
    <div class="footer">
        <p class="footer-brand"><span>Faktura</span>lista &mdash; {{ __('emails.trial_reminder.footer_tagline') }}</p>
        <p class="footer-links">
            <a href="{{ url('/') }}">fakturalista.com</a>
            &nbsp;&middot;&nbsp;
            <a href="{{ url('/contact') }}">{{ __('emails.trial_reminder.footer_support') }}</a>
            &nbsp;&middot;&nbsp;
            <a href="{{ url('/pricing') }}">{{ __('emails.trial_reminder.footer_pricing') }}</a>
        </p>
        <p style="font-size:11px;color:#d1d5db;margin-top:12px;">
            {{ __('emails.trial_reminder.footer_note') }}
        </p>
    </div>
